...
!(function () {
  ...
  // when like-post-btn clicked, get the data-postid attribute
  // and send a post request to /post/like
  // to like the post
  var likePostBtns = document.querySelectorAll('.like-post-btn')
  likePostBtns.forEach(btn => {
    btn.addEventListener('click', function () {
      ...
      // send post request to /post/like
      ...
      fetch('/post/like', requestOptions)
        .then(response => response.text())
        .then(result => {
          // add plus one to the like count
          var likeCount = btn.querySelector('.like-count')
          //console.log('likeCount', likeCount)
          var likeCountVal = parseInt(likeCount.innerText.replace('(', '').replace(')', ''))
          if (btn.classList.contains('active')) {
            likeCount.innerHTML = '(' + (likeCountVal + 1) + ')'
          } else {
            likeCount.innerHTML = '(' + (likeCountVal - 1) + ')'
          }
        })
    ...
    // when comment textarea focused, show the comment btn
    // var commentTextarea = document.querySelector('.comment-input')
    let commentTextarea = document.querySelectorAll('.comment-input')
    commentTextarea.forEach(textarea => {
      textarea.addEventListener('focus', function () {
        var commentBtn = textarea.parentElement.nextElementSibling
        //console.log('commentBtn', commentBtn)
        // show the comment btn
    ...
    //     var commentBtn = commentTextarea.parentElement.nextElementSibling
    //     console.log('commentBtn', commentBtn)
    //     // show the comment btn
    //     commentBtn.classList.remove('d-none')
    //   })
...
var e = {
  init: function () {
    e.preLoader(), e.navbarDropdownHover(), e.tinySlider(), e.toolTipFunc(), e.popOverFunc(), e.videoPlyr(), e.lightBox(), e.darkMode(), e.sidebarToggleStart(), e.sidebarToggleEnd(), e.choicesSelect(), e.autoResize(), e.DropZone(), e.flatPicker(), e.avatarImg(), e.customScrollbar(), e.toasts(), e.pswMeter(), e.fakePwd()
  },
      ...
      if (e.isVariableDefined(e.select('.dropzone-default'))) {
        e.selectAll('.dropzone-default').forEach(e => {
          const a = e.dataset.dropzone ? JSON.parse(e.dataset.dropzone) : {},
            b = {
              url: '/post/ImageUpload', // Change this URL to your actual image upload code
              ...
              init: function () {
                ...
                this.on('success', function (file, response) {
                  ...
                  var hiddenInput = document.querySelector('#image-uploaded-url')
                  hiddenInput.value = response.file.url
                })
              }
            },
            c = {
  ...
  // END: Fake Password
  submitNewTextPost: function () {
    let textareaElem = document.querySelector('#post-write')
  ...
  deletePost: function (id) {
    ...
    if (prompt) {
      ...
      fetch('/post/delete', requestOptions)
        .then(response => response.text())
        .then(result => {
          // reload the page
          window.location.reload()
  ...
  submitComment: function (clickedElement) {
    let postId = clickedElement.getAttribute('data-comment-postid')

    let commentTextarea = clickedElement.parentElement[0]
    ...
    fetch('/comment/create', requestOptions)
      .then(response => response.text())
      .then(result => {
        // reload the page
        window.location.reload()
  ...
  sendFriendRequest: function (clickedElement) {
    let friendId = clickedElement.getAttribute('data-friend-id')
    let myHeaders = new Headers()
    myHeaders.append('Content-Type', 'application/json')
    let requestOptions = {
      method: 'POST',
    ...
    fetch('/friends/create', requestOptions)
      .then(response => response.text())
      .then(result => {
        // reload the page
        window.location.reload()
  ...
  deleteFriend: function (clickedElement) {
    let friendId = clickedElement.getAttribute('data-friend-id')
    let myHeaders = new Headers()
    myHeaders.append('Content-Type', 'application/json')
    let requestOptions = {
    ...
    fetch('/friends/delete', requestOptions)
      .then(response => response.text())
      .then(result => {
        // reload the page
        window.location.reload()
  ...
  acceptRequest: function (clickedElement) {
    let requestId = clickedElement.getAttribute('data-request-id')
    let myHeaders = new Headers()
    myHeaders.append('Content-Type', 'application/json')
    let requestOptions = {
      method: 'POST',
    ...
    fetch('/friends/accept', requestOptions)
      .then(response => response.text())
      .then(result => {
        // reload the page
        window.location.reload()
  ...
  rejectRequest: function (clickedElement) {
    let requestId = clickedElement.getAttribute('data-request-id')
    let myHeaders = new Headers()
    myHeaders.append('Content-Type', 'application/json')
    let requestOptions = {
    ...
    fetch('/friends/decline', requestOptions)
      .then(response => response.text())
      .then(result => {
        // reload the page
        window.location.reload()
  ...
  cancelRequest: function (clickedElement) {
    ...
    fetch('/friends/cancel', requestOptions)
      .then(response => response.text())
      .then(result => {
        // reload the page
        window.location.reload()
